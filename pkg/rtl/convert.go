			...
			if rtlmap.WallPlane[x][y] == elevatorDoorTile {
				if x > 1 && rtlmap.WallPlane[x-2][y] == elevatorSwitchTile {
					...
					elevators[linkCode] = append(elevators[linkCode], ElevatorNode{
						Floor:  rtlmap.CookedWallGrid[x-1][y],
						Switch: rtlmap.CookedWallGrid[x-2][y],
					})
					...
					elevators[linkCode] = append(elevators[linkCode], ElevatorNode{
						Floor:  rtlmap.CookedWallGrid[x][y-1],
						Switch: rtlmap.CookedWallGrid[x][y-2],
					})
...
func ConvertRTLMapToQuakeMapFile(rtlmap *RTLMapData, textureWad string, scale float64, dusk bool) *quakemap.QuakeMap {
		...
		for j := 0; j < 128; j++ {
			...
			if wallInfo.Type == WALL_Regular || wallInfo.Type == WALL_AnimatedWall || wallInfo.Type == WALL_Elevator {
				...
				if infoVal == 1 || (infoVal >= 4 && infoVal <= 9) {
					...
					if wallInfo.Tile == 0x4c {
						// do not render elevator switches
						continue
					}
					// plain ol' column
	...
	for doornum, door := range rtlmap.GetDoors() {
		...
		// move upward when open, move sideways if elevator door
		if door.Tiles[0].Tile == 0x66 {
